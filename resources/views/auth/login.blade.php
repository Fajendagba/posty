...
@section('content')
    <div class="flex justify-center">
        <div class="w-4/12 bg-white p-6 rounded-lg">
            ...
            @if (session('status')) {{-- Or you could use @if (session()->has('status'))  status is a variable from the login controller--}}
                <div class="bg-red-500 p-2 rounded-lg mb-2 text-white text-center">
                    {{ session('status') }}
                </div>
            @endif
